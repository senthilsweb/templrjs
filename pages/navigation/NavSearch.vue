<template>
  <div>
    <form name="frmSearchNavigation" id="frmSearchNavigation" ref="frmSearchNavigation" v-on:submit.prevent class="space-y-8 divide-y divide-gray-200">
      <div v-if="isShowSearchNavigationVisible" class="px-2 pt-2 border-b space-y-2">
        ...
        <div class="sm:col-span-6">
          ...
          <div class="sm:col-span-6">
            <Dropdownlist :data="{ data: usePropertiesStore().properties_by_parent_code('navigation-by-modules') }" v-on:change="handleSelectedInNavigationSearch_module" show_label="true" v-model="data.nav_module_code" name="nav_module_code" label="Module" :selected_value="data.nav_module_code" />
          </div>
          <div class="sm:col-span-6">
        ...
        <!--<div class="sm:col-span-6">
          <Dropdownlist :data="{ data: record_status.documents }" v-on:change="handleSelectedInNavigation_record_status" show_label="true" v-model="data.record_status" name="record_status" label="Record Status" :selected_value="data.record_status" />
        </div>
        <div class="sm:col-span-6">
          <Dropdownlist :data="{ data: limits.documents }" v-on:change="handleSelectedInNavigationLimit" show_label="true" v-model="data.limit" name="limit" label="Limit #. of results to" :selected_value="data.limit" />
        </div>-->
        <div class="py-3 text-right space-x-4">
          ...
          <button @click="useNuxtApp().$bus.$emit('evtSearchNavigation', data)" type="submit" class="zyn-button">Search</button>
        </div>
      </div>
    </form>
  </div>
...
function handleSelectedInNavigation_record_status(e) {
  data.value.record_status = e.target.value;
}
function handleSelectedInNavigationLimit(e) {
