<!--
  ...
  description: This component is used to create and update [applications] entity.
  It is used in the following pages:
  - pages/things/applications/index.vue
-->
<template>
  <section v-if="isUpsertApplicationsVisible" class="fixed inset-0 overflow-hidden" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
            ...
            <div class="flex-1 h-0 overflow-y-auto">
              <header class="space-y-1 py-6 px-4 bg-gray-200 bg-opacity-50 sm:px-6">
                <div class="flex items-center justify-between space-x-3">
                  <h2 class="text-lg leading-7 font-medium text-gray-700">Applications</h2>
                  <div class="h-7 flex items-center">
                    <button v-on:click="isUpsertApplicationsVisible = false" aria-label="Close panel" class="rounded-md bg-primary-700 text-primary-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white">
              ...
              <div v-if="!progress" class="flex-1 flex flex-col justify-between">
                <div class="px-4 divide-y divide-gray-200 sm:px-6">
                  <div class="space-y-6 pt-6 pb-5">
                    <form name="frmApplications" id="frmApplications" class="space-y-8 divide-y divide-gray-200" @submit.prevent="upsertApplications">
                        ...
                        <div class="bg-white space-y-6">
                          <div class="grid grid-cols-1 gap-y-3 gap-x-4 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-3">
                              <div class="mt-1">
                                <Switch id="is_active" name="is_active" v-model="data.is_active" :class="data.is_active ? 'zyn-swatch-button zyn-swatch-selected' : 'zyn-swatch-button zyn-swatch-unselected'">
                            ...
                            <div class="sm:col-span-3">
                              <div class="mt-1">
                                <Switch id="is_archived" name="is_archived" v-model="data.is_archived" :class="data.is_archived ? 'zyn-swatch-button zyn-swatch-selected' : 'zyn-swatch-button zyn-swatch-unselected'">
...
useNuxtApp().$bus.$on('evtDeleteApplications', (data) => {
  deleteApplications(data._cells[0].data);
});
