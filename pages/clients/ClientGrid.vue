...
const api = '/api/clients/'; /*To-be customized for each entity */
...
onMounted(() => {
  grid.value = new Grid({
        ...
        formatter: (cell, row) => {
          return h(
            'button',
            {
              ...
              onClick: () => useNuxtApp().$bus.$emit('evtUpsertClient', row),
            },
            'Edit'
          );
        },
    ...
    search: true,
    fixedHeader: true,
    data: Clients._rawValue.documents,
  });
...
export default {
  ...
  methods: {
    ...
    async searchClients() {
      this.progress = true;
      try {
        let consultant_list = await useFetch('/api/clients', {
          method: 'post',
          initialCache: false,
          body: {
  ...
  created() {
    useNuxtApp().$bus.$on('evtSearchClient', (data) => {
      this.filter = data;
      this.searchClients();
    });
  },
