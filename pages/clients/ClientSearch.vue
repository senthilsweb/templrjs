<template>
	<div>
		<form name="frmSearchClient" id="frmSearchClient" ref="frmSearchClient" v-on:submit.prevent
			class="space-y-8 divide-y divide-gray-200">
			<div v-if="isShowSearchClientVisible" class="px-2 pt-2 border-b space-y-2">
				...
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: record_status.documents }"
						v-on:change="handleSelectedInClient_record_status" show_label="true"
				...
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: mark_as_delete.documents }"
						v-on:change="handleSelectedInClient_mark_as_delete" show_label="true"
						v-model="data.mark_as_delete" name="mark_as_delete" label="Mark for Deletion"
						:selected_value="data.mark_as_delete" />
				</div>
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: limits.documents }" v-on:change="handleSelectedInClientLimit"
				...
				<div class="py-3 text-right space-x-4">
					...
					<button @click="useNuxtApp().$bus.$emit('evtSearchClient', data)" type="submit"
...
export default {
	...
	data() {
		return {
			isShowSearchClientVisible: false,
			data: {},
		};
	},
	methods: {
		...
		handleSelectedInClientLimit(e) {
			this.data.limit = e.target.value;
		},
	},
	created() {
		useNuxtApp().$bus.$on("evtShowSearchClient", (data) => {
			this.isShowSearchClientVisible = !this.isShowSearchClientVisible;
		});
	},
};
</script>
