...
-->
<template>
  <section v-if="isUpsertProfileVisible" class="fixed inset-0 overflow-hidden" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
            ...
            <div class="flex-1 h-0 overflow-y-auto">
              <header class="space-y-1 py-6 px-4 bg-primary-700 sm:px-6">
                <div class="flex items-center justify-between space-x-3">
                  <h2 class="text-lg leading-7 font-medium text-white">Profile</h2>
                  <div class="h-7 flex items-center">
                    <button v-on:click="isUpsertProfileVisible = false" aria-label="Close panel" class="rounded-md bg-primary-700 text-primary-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white">
                ...
                <div>
                  <p class="text-sm leading-5 text-primary-300">Your basic profile details visible to general public.</p>
                </div>
              </header>
              <!--Progress bar (start)-->
              ...
              <div v-if="!progress" class="flex-1 flex flex-col justify-between">
                <div class="px-4 divide-y divide-gray-200 sm:px-6">
                  <div class="space-y-6 pt-6 pb-5">
                    <form name="frmProfiles" id="frmProfiles" class="space-y-8 divide-y divide-gray-200" @submit.prevent="upsertProfile">
...
useNuxtApp().$bus.$on('evtUpsertProfile', (args) => {
  //alert(useSupabaseUser().value.id)
...
useNuxtApp().$bus.$on('evtDeleteProfile', (data) => {
...
async function upsertProfile() {
  try {
    ...
    await useFetch(api + useSupabaseUser().value.id, {
      method: 'put',
      body: data.value,
      initialCache: false,
      onResponse({ request, response, options }) {
