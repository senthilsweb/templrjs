<template>
	<section v-if="isUpsertTypeVisible" class="fixed inset-0 overflow-hidden" aria-labelledby="slide-over-title"
					...
					<div class="h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl">
						<div class="flex-1 h-0 overflow-y-auto">
							<header class="space-y-1 py-6 px-4 bg-primary-200 bg-opacity-50 sm:px-6">
								<div class="flex items-center justify-between space-x-3">
									<h2 class="text-lg leading-7 font-medium text-gray-700">Type</h2>
									<div class="h-7 flex items-center">
										<button v-on:click="closeTypePanel()" aria-label="Close panel"
								...
								<div>
									<p class="text-sm leading-5 text-gray-500">Create or update Type</p>
								</div>
							</header>
							<!--Progress bar (start)-->
							...
							<div v-if="!progress" class="flex-1 flex flex-col justify-between">
								<div class="px-4 divide-y divide-gray-200 sm:px-6">
									<div class="space-y-6 pt-6 pb-5">
										<form name="frmType" id="frmType" class="space-y-8 divide-y divide-gray-200"
											@submit.prevent="upsertType">
													...
													<div class="grid grid-cols-1 gap-y-3 gap-x-4 sm:grid-cols-6">
														...
														<div class="sm:col-span-6">
															<Dropdownlist :data="{ data: record_status.documents }"
																v-on:change="handleSelectedInType_record_status"
														...
														<div class="sm:col-span-6">
															<Dropdownlist :data="{ data: mark_as_delete.documents }"
																v-on:change="handleSelectedInType_mark_as_delete"
						...
						<div class="w-full flex-shrink-0 px-4 py-4 space-x-4 flex justify-end">
							...
							<span v-if="isDeleteButtonVisible" class="inline-flex rounded-md shadow-sm">
								<button v-on:click="useNuxtApp().$bus.$emit('evtDeleteType', data)" type="button"
							...
							<span class="inline-flex rounded-md shadow-sm">
								<button v-on:click="upsertType" type="submit"
...
export default {
	...
	created() {
		...
		//2) Delete Type record and then rebind datatable
