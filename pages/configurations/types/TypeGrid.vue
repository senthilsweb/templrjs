...
const api = "/api/types/"; /*To-be customized for each entity */
...
const { data: Types } = await useAsyncData("Types-list-" + Math.random, () =>
	$fetch(api, {
		initialCache: false,
		method: "post",
...
onMounted(() => {
	grid.value = new Grid({
				...
				formatter: (cell, row) => {
					return h(
						"button",
						{
							...
							onClick: () => useNuxtApp().$bus.$emit("evtUpsertType", row),
						},
						"Edit"
					);
				},
		...
		search: true,
		fixedHeader: true,
		data: Types._rawValue.documents,
	});
...
export default {
	...
	methods: {
		...
		async searchTypes() {
			this.progress = true;
			try {
				let consultant_list = await useFetch("/api/types", {
					method: "post",
					initialCache: false,
					body: {
	...
	created() {
		useNuxtApp().$bus.$on("evtSearchType", (data) => {
			this.filter = data;
			this.searchTypes();
		});
	},
