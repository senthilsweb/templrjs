<template>
	<div>
		<form name="frmSearchType" id="frmSearchType" ref="frmSearchType" v-on:submit.prevent
			class="space-y-8 divide-y divide-gray-200">
			<div v-if="isShowSearchTypeVisible" class="px-2 pt-2 border-b space-y-2">
				...
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: record_status.documents }"
						v-on:change="handleSelectedInType_record_status" show_label="true" v-model="data.record_status"
						name="record_status" label="Record Status" :selected_value="data.record_status" />
				</div>
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: mark_as_delete.documents }"
						v-on:change="handleSelectedInType_mark_as_delete" show_label="true"
						v-model="data.mark_as_delete" name="mark_as_delete" label="Mark for Deletion"
						:selected_value="data.mark_as_delete" />
				</div>
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: limits.documents }" v-on:change="handleSelectedInTypeLimit"
				...
				<div class="py-3 text-right space-x-4">
					...
					<button @click="useNuxtApp().$bus.$emit('evtSearchType', data)" type="submit"
...
export default {
	...
	methods: {
		...
		handleSelectedInTypeLimit(e) {
			this.data.limit = e.target.value;
		},
	},
	created() {
		useNuxtApp().$bus.$on("evtShowSearchType", (data) => {
			this.isShowSearchTypeVisible = !this.isShowSearchTypeVisible;
		});
	},
};
</script>
