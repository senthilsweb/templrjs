...
const api = '/api/configurations/'; /*To-be customized for each entity */
...
const { data: Configurations } = await useAsyncData('Configurations-list-' + Math.random, () =>
  $fetch(api, {
    initialCache: false,
    method: 'post',
...
onMounted(() => {
  grid.value = new Grid({
        ...
        formatter: (cell, row) => {
          return h(
            'button',
            {
              ...
              onClick: () => useNuxtApp().$bus.$emit('evtUpsertConfiguration', row),
            },
            'Edit'
          );
        },
    ...
    search: true,
    fixedHeader: true,
    data: Configurations._rawValue.documents,
  });
...
export default {
  ...
  methods: {
    ...
    buildMongoQueryFilterCriteria(args) {
      //console.log("configuration serach args=", JSON.stringify(args));
      let filter = {};
      if (_.isString(args.name) && !useNuxtApp().$s.isBlank(args.name)) {
        filter['name'] = { $regex: '.*' + args.code + '.*', $options: 'i' };
      }
      if (_.isString(args.property_type) && !useNuxtApp().$s.isBlank(args.property_type)) {
        filter['property_type'] = { $eq: args.property_type };
      }
    ...
    async searchConfigurations() {
      this.progress = true;
      try {
        let consultant_list = await useFetch('/api/configurations', {
          method: 'post',
          initialCache: false,
          body: {
  ...
  created() {
    useNuxtApp().$bus.$on('evtSearchConfiguration', (data) => {
      this.filter = data;
