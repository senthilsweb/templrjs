<template>
  <div>
    <form name="frmSearchConfiguration" id="frmSearchConfiguration" ref="frmSearchConfiguration" v-on:submit.prevent class="space-y-8 divide-y divide-gray-200">
      <div v-if="isShowSearchConfigurationVisible" class="px-2 pt-2 border-b space-y-2">
        <h2 v-if="this.heading" class="text-lg leading-7 font-medium">
          {{ this.heading }}
        </h2>
        <div class="sm:col-span-6">
          <Dropdownlist :data="{ data: configuration_types.documents }" v-on:change="handleSelectedInConfiguration_property_type" show_label="true" v-model="data.property_type" name="property_type" label="Configuration Type" :selected_value="data.property_type" />
        </div>
        <div class="sm:col-span-6">
        ...
        <!--
				<div class="sm:col-span-6">
					<Dropdownlist :data="{data: record_status.documents}" v-on:change="handleSelectedInConfiguration_record_status" show_label="true" v-model="data.record_status" name="record_status" label="Record Status" :selected_value="data.record_status" />
				</div>
				<div class="sm:col-span-6">
					<Dropdownlist :data="{data: mark_as_delete.documents}" v-on:change="handleSelectedInConfiguration_mark_as_delete" show_label="true" v-model="data.mark_as_delete" name="mark_as_delete" label="Mark for Deletion" :selected_value="data.mark_as_delete" />
				</div>
-->
        <div class="sm:col-span-6">
          <Dropdownlist :data="{ data: limits.documents }" v-on:change="handleSelectedInConfigurationLimit" show_label="true" v-model="data.limit" name="limit" label="Limit #. of results to" :selected_value="data.limit" />
        </div>
        <div class="py-3 text-right space-x-4">
          ...
          <button @click="useNuxtApp().$bus.$emit('evtSearchConfiguration', data)" type="submit" class="zyn-button">Search</button>
        </div>
      </div>
    </form>
  </div>
