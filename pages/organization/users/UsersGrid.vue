<!--
  ...
  description: This is the grid component for the [Users] entity
-->
<template>
...
useNuxtApp().$bus.$on('evtSearchUsers', (data) => {
  searchUsers(data);
});
...
async function searchUsers(args) {
  ...
  try {
    let response = await useAsyncData('Users-list-' + Math.random, () =>
...
onMounted(() => {
  grid.value = new Grid({
    ...
    { id: 'id', name: 'Id.', width: '50px',hidden:true },
      ...
      { id: 'first_name', name: 'First Name', width: '50px' },
    { id: 'last_name', name: 'Last Name', width: '50px'},
      /*{
        name: 'Status', 
        data: (row) => {
        ...
        formatter: (cell, row) => {
          return h(
            'button',
            {
              ...
              onClick: () => useNuxtApp().$bus.$emit('evtDeleteUser', row),
            },
            'Delete'
          );
        },
        ...
        formatter: (cell, row) => {
          return h(
            'button',
            {
              ...
              onClick: () => useNuxtApp().$bus.$emit('evtUpsertUser', row),
            },
            'Edit'
          );
        },
