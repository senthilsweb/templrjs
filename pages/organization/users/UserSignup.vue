            ...
            <div class="flex-1 h-0 overflow-y-auto">
              ...
              <div v-if="!progress" class="flex-1 flex flex-col justify-between">
                <div class="px-4 divide-y divide-gray-200 sm:px-6">
                  <div class="space-y-6 pt-6 pb-5">
                    <form name="frmUsers" id="frmUsers" class="space-y-8 divide-y divide-gray-200" @submit.prevent="newUserSignUp">
                        ...
                        <div class="bg-white space-y-6">
                          <div class="grid grid-cols-1 gap-y-3 gap-x-4 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-6">
                              ...
                              <div class="mt-1">
                                <input type="password" id="password" name="password" v-model="data.password" class="block w-full pl-5 focus:ring-primary-600 focus:border-primary-600 border-gray-300" />
                              </div>
                            </div>
  ...
  try {
    ...
    await useFetch(api + 'signup', {
      ...
      onResponse({ request, response, options }) {
        ...
        if (response._data) {
          if (response._data.status==500) {
            useNuxtApp().$toast.show({ type: 'danger', message: response._data.message, timeout: 6 });
          }
          else if (response._data.status==200) {
            useNuxtApp().$toast.show({ type: 'success', message: response._data.message, timeout: 6 });
