...
const api = '/api/properties/'; /*To-be customized for each entity */
...
const { data: Propertiess } = await useAsyncData('Propertiess-list-' + Math.random, () =>
...
async function searchPropertiess(args) {
  ...
  try {
    let response = await useAsyncData('Propertiess-list-' + Math.random, () =>
...
onMounted(() => {
  grid.value = new Grid({
    columns: [
      ...
      { id: 'parent_code', name: 'Parent Code', width: '100px' },
        ...
        formatter: (cell, row) => {
          return h(
            'button',
            {
              ...
              onClick: () => useNuxtApp().$bus.$emit('evtDeleteProperties', row),
            },
            'Delete'
          );
        },
        ...
        formatter: (cell, row) => {
          return h(
            'button',
            {
              ...
              onClick: () => useNuxtApp().$bus.$emit('evtUpsertProperties', row),
            },
            'Edit'
          );
        },
