  ...
  <NuxtLayout name="landing">
    ...
    <div class="container p-4 lg:p-0 max-w-5xl mx-auto sm:grid grid-cols-12 gap-x-12">
      <div class="col-span-12 lg:col-span-8">
        ...
        <TemplrJSSocialCard  v-if="page.type=='Blog'" :title="page.title" :description="page.description" :tags="page.tags" :author="page.author" :date="page.date" color="teal" :type="page.type" />
...
const page = ref({});
console.log(`${useRuntimeConfig().public.TEMPLRJS_WEB_ROOT_PATH}/api/_content/query?_params={"where":{"_path":{"$eq":"/blog/${useRoute().params.title}"}},"only":["title","author","date","description","body","coverimage","type","tags","published"]}`)

const data = await $fetch(`${useRuntimeConfig().public.TEMPLRJS_WEB_ROOT_PATH}/api/_content/query?_params={"where":{"_path":{"$eq":"/blog/${useRoute().params.title}"}},"only":["title","author","date","description","body","coverimage","type","tags","published"]}`);

if (data && _.isArray(data)) {
  page.value = data[0];
  const config = useCloudinaryConfig(page.value.title, useRuntimeConfig().public.CLOUDINARY_BASE_URL, 'v1711924071/senthilsweb-scl-card-template_cyxogj.png');
...
useHead({
  title: `${page.value.title} - ${websiteBrand}`, // Combining blog title with website brand
  meta: [
    { name: 'description', content: `${page.value.title} - Learn more about how we simplify complex technologies at ${websiteBrand}.` },
    { property: 'og:description', content: `${page.value.title} - Discover cutting-edge solutions and insights at ${websiteBrand}.` },
    { property: 'og:title', content: `${page.value.title} - ${websiteBrand}` },
    { property: 'og:image', content: cloudinaryURL.value },
    { name: 'twitter:title', content: `${page.value.title} - ${websiteBrand}` },
    { name: 'twitter:image', content: cloudinaryURL.value },
    { name: 'twitter:card', content: 'summary_large_image' },
    { name: 'twitter:description', content: `${page.value.title} - Explore innovative solutions at ${websiteBrand}.` },
  ],
});
</script>
