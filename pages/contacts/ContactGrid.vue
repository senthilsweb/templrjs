...
const api = "/api/contacts/"; /*To-be customized for each entity */
...
const { data: Contacts } = await useAsyncData("Contacts-list-" + Math.random, () =>
	$fetch(api, {
		initialCache: false,
		method: "post",
...
onMounted(() => {
	grid.value = new Grid({
			...
			/*{
				...
				data: (row) => {
					...
					return h(
						...
						row.mark_as_delete ? (row.mark_as_delete.toLowerCase() == "yes" ? "Delete Marked" : "") : ""
					);
				},
				sort: false,
				width: "25px",
				...
				formatter: (cell, row) => {
					return h(
						"button",
						{
							...
							onClick: () => useNuxtApp().$bus.$emit("evtUpsertContact", row),
						},
						"Edit"
					);
				},
		...
		search: true,
		fixedHeader: true,
		data: Contacts._rawValue ? Contacts._rawValue.documents : [],
	});
...
export default {
	...
	methods: {
		...
		buildMongoQueryFilterCriteria(args) {
			...
			if (_.isString(args.contact_type) && !useNuxtApp().$s.isBlank(args.contact_type)) {
				filter["contact_type"] = { $eq: args.contact_type };
			}
			if (_.isString(args.client) && !useNuxtApp().$s.isBlank(args.client)) {
				filter["client"] = { $eq: args.client };
			}
		...
		async searchContacts() {
			this.progress = true;
			try {
				let consultant_list = await useFetch("/api/contacts", {
					method: "post",
					initialCache: false,
					body: {
	...
	created() {
		useNuxtApp().$bus.$on("evtSearchContact", (data) => {
			this.filter = data;
			this.searchContacts();
		});
	},
