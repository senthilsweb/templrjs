<template>
	<section v-if="isUpsertContactVisible" class="fixed inset-0 overflow-hidden" aria-labelledby="slide-over-title"
					...
					<div class="h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl">
						<div class="flex-1 h-0 overflow-y-auto">
							<header class="space-y-1 py-6 px-4 bg-primary-200 bg-opacity-50 sm:px-6">
								<div class="flex items-center justify-between space-x-3">
									<h2 class="text-lg leading-7 font-medium text-gray-700">Contact</h2>
									<div class="h-7 flex items-center">
										<button v-on:click="closeContactPanel()" aria-label="Close panel"
								...
								<div>
									<p class="text-sm leading-5 text-gray-500">Create or update Contact</p>
								</div>
							</header>
							<!--Progress bar (start)-->
								...
								<div class="px-4 divide-y divide-gray-200 sm:px-6">
									<div class="space-y-6 pt-6 pb-5">
										<form name="frmContact" id="frmContact"
											class="space-y-8 divide-y divide-gray-200" @submit.prevent="upsertContact">
											<div class="sm:overflow-hidden">
												<div class="bg-white space-y-6">
													<div class="grid grid-cols-1 gap-y-3 gap-x-4 sm:grid-cols-6">
														<div class="sm:col-span-6">
															<h2 class="font-medium text-primary-700">Contact Information
															</h2>
														</div>
														...
														<div class="sm:col-span-6">
															...
															<p class="mt-2 text-sm text-gray-500">Name of the contact
																person</p>
														</div>
														<div class="sm:col-span-6">
															<label for="email"
														...
														<div class="sm:col-span-6">
															<h2 class="font-medium text-primary-700">Address Information
															</h2>
														</div>
														...
														<div class="sm:col-span-6">
															<Dropdownlist :data="{ data: countries.documents }"
																v-on:change="handleSelectedInClient_country"
																show_label="true" v-model="data.address.country"
																name="country" label="Country"
																:selected_value="data.address.country" />
														</div>
														<div class="sm:col-span-6">
															<h2 class="font-medium text-primary-700">Contact Type
																Information</h2>
														</div>

														<div class="sm:col-span-6">
															<Dropdownlist :data="{ data: contact_types.documents }"
																v-on:change="handleSelectedInContact_contact_type"
																show_label="true" v-model="data.contact_type"
																name="contact_type" label="Type"
																:selected_value="data.contact_type" />
														</div>
														<div v-if="data.contact_type == 'Active Client'"
															class="sm:col-span-6">
															<Dropdownlist :data="{ data: clients.documents }"
																v-on:change="handleSelectedInConsultantClient"
														...
														<div class="sm:col-span-6">
															<Dropdownlist :data="{ data: record_status.documents }"
																v-on:change="handleSelectedInContact_record_status"
														...
														<div class="sm:col-span-6">
															<Dropdownlist :data="{ data: mark_as_delete.documents }"
																v-on:change="handleSelectedInContact_mark_as_delete"
						...
						<div class="w-full flex-shrink-0 px-4 py-4 space-x-4 flex justify-end">
							...
							<span v-if="isDeleteButtonVisible" class="inline-flex rounded-md shadow-sm">
								<button v-on:click="useNuxtApp().$bus.$emit('evtDeleteContact', data)" type="button"
							...
							<span class="inline-flex rounded-md shadow-sm">
								<button v-on:click="upsertContact" type="submit"
...
export default {
	...
	created() {
		...
		//2) Delete Contact record and then rebind datatable
