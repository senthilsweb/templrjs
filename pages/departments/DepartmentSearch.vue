<template>
	<div>
		<form name="frmSearchDepartment" id="frmSearchDepartment" ref="frmSearchDepartment" v-on:submit.prevent
			class="space-y-8 divide-y divide-gray-200">
			<div v-if="isShowSearchDepartmentVisible" class="px-2 pt-2 border-b space-y-2">
				...
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: record_status.documents }"
						v-on:change="handleSelectedInDepartment_record_status" show_label="true"
				...
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: mark_as_delete.documents }"
						v-on:change="handleSelectedInDepartment_mark_as_delete" show_label="true"
						v-model="data.mark_as_delete" name="mark_as_delete" label="Mark for Deletion"
						:selected_value="data.mark_as_delete" />
				</div>
				<div class="sm:col-span-6">
					<Dropdownlist :data="{ data: limits.documents }" v-on:change="handleSelectedInDepartmentLimit"
				...
				<div class="py-3 text-right space-x-4">
					...
					<button @click="useNuxtApp().$bus.$emit('evtSearchDepartment', data)" type="submit"
...
export default {
	...
	data() {
		return {
			isShowSearchDepartmentVisible: false,
			data: {},
		};
	},
	methods: {
		...
		handleSelectedInDepartmentLimit(e) {
			this.data.limit = e.target.value;
		},
	},
	created() {
		useNuxtApp().$bus.$on("evtShowSearchDepartment", (data) => {
			this.isShowSearchDepartmentVisible = !this.isShowSearchDepartmentVisible;
		});
	},
};
</script>
