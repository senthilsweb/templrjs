...
const api = "/api/departments/"; /*To-be customized for each entity */
...
onMounted(() => {
	grid.value = new Grid({
				...
				formatter: (cell, row) => {
					return h(
						"button",
						{
							...
							onClick: () => useNuxtApp().$bus.$emit("evtUpsertDepartment", row),
						},
						"Edit"
					);
				},
		...
		search: true,
		fixedHeader: true,
		data: Departments._rawValue ? Departments._rawValue.documents : [],
	});
...
export default {
	...
	methods: {
		...
		async searchDepartments() {
			this.progress = true;
			try {
				let consultant_list = await useFetch("/api/departments", {
					method: "post",
					initialCache: false,
					body: {
	...
	created() {
		useNuxtApp().$bus.$on("evtSearchDepartment", (data) => {
			this.filter = data;
