<!--
  ...
  description: This is the grid component for the [Profiles] entity
-->
<template>
...
const api = '/api/profiles'; /*To-be customized for each entity */
const grid = useState('grid');
const progress = ref(false);
const search_query_string = ref('');
useNuxtApp().$bus.$on('evtSearchProfiles', (data) => {
  searchProfiles(data);
});

const gridRef = ref();

const { data: Profiles } = await useAsyncData('Profiles-list-' + Math.random, () =>
  $fetch(api, {
    initialCache: false,
    method: 'get',
  })
...
async function searchProfiles(args) {
  ...
  try {
    let response = await useAsyncData('Profiles-list-' + Math.random, () =>
...
onMounted(() => {
  grid.value = new Grid({
    ...
    { id: 'id', name: 'Id.', width: '50px',hidden:true },
      { id: 'email', name: 'email', width: '50px'},
      {
        name: 'Status', 
        data: (row) => {
        ...
        formatter: (cell, row) => {
          return h(
            'button',
            {
              ...
              onClick: () => useNuxtApp().$bus.$emit('evtDeleteProfiles', row),
            },
            'Delete'
          );
        },
        ...
        formatter: (cell, row) => {
          return h(
            'button',
            {
              ...
              onClick: () => useNuxtApp().$bus.$emit('evtUpsertProfiles', row),
            },
            'Edit'
          );
        },
