...
function generateTransformationString(transformation) {
  ...
  if (transformation.textOverlay) {
    ...
    // Add default color black if no color is specified
    const color = textOverlay.color || 'black';
    parts.push(`l_text:${textOverlay.font || 'Arial'}_${textOverlay.size || 60}_${textOverlay.weight || 'bold'}_co_${color}:${encodeURIComponent(textOverlay.content || 'Default Text')}`, `g_${textOverlay.gravity || 'south'}`, `x_${textOverlay.xOffset || 10}`, `y_${textOverlay.yOffset || 10}`);
  }
