...
const __default__ = {
  ...
  methods: {
    ...
    async upsertRole(args) {
      try {
          ...
          const { data: res } = await useFetch(api + "create", {
            ...
            onResponse({ request, response, options }) {
              if (response._data.insertedId) {
                useNuxtApp().$toast.show({ type: "success", message: `Role [${response._data.insertedId}] added successfully`, timeout: 6 });
              }
            }
          }, "$S3gMTQq7A6");
        } else {
        ...
        this.progress = false;
        useNuxtApp().$bus.$emit("evtSearchRole", { filter: {}, limit: 10 });
        this.data = {};
    ...
    async deleteRole() {
      try {
        await useFetch("/api/roles/" + this.data._id, {
          method: "delete",
          body: {},
          initialCache: false,
          onResponse({ request, response, options }) {
            if (response._data.deletedCount == 1) {
              ...
              this.progress = false;
              this.isUpsertRoleVisible = false;
              useNuxtApp().$bus.$emit("evtSearchRole", { filter: {}, limit: 10 });
              this.data = {};
            } else {
              this.progress = false;
    ...
    handleSelectedInRole_record_status(e) {
      this.data.record_status = e.target.value;
    },
    handleSelectedInRole_mark_as_delete(e) {
  ...
  created() {
    useNuxtApp().$bus.$on("evtUpsertRole", (data) => {
      if (data !== void 0) {
        this.getRole(data._cells[0].data);
      }
      this.isUpsertRoleVisible = !this.isUpsertRoleVisible;
    });
...
const _sfc_main = /* @__PURE__ */ Object.assign(__default__, {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (_ctx.isUpsertRoleVisible) {
        _push(`<section${ssrRenderAttrs(mergeProps({
          class: "fixed inset-0 overflow-hidden",
          "aria-labelledby": "slide-over-title",
          role: "dialog",
          "aria-modal": "true"
        }, _attrs))}><div class="absolute inset-0 overflow-hidden"><div class="absolute inset-0" aria-hidden="true"></div><div class="absolute inset-y-0 right-0 pl-10 max-w-full flex"><div class="w-screen max-w-md"><div class="h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl"><div class="flex-1 h-0 overflow-y-auto"><header class="space-y-1 py-6 px-4 bg-primary-200 bg-opacity-50 sm:px-6"><div class="flex items-center justify-between space-x-3"><h2 class="text-lg leading-7 font-medium text-gray-700">Role</h2><div class="h-7 flex items-center"><button aria-label="Close panel" class="rounded-md bg-primary-700 text-primary-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div></div><div><p class="text-sm leading-5 text-gray-500">Create or update Role</p></div></header>`);
        if (_ctx.progress) {
        ...
        if (!_ctx.progress) {
          ...
          _push(ssrRenderComponent(_component_Dropdownlist, {
            ...
            onChange: _ctx.handleSelectedInRole_record_status,
            show_label: "true",
          ...
          _push(ssrRenderComponent(_component_Dropdownlist, {
            ...
            onChange: _ctx.handleSelectedInRole_mark_as_delete,
            show_label: "true",
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/roles/RoleUpsert.vue");
