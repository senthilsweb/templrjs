...
const __default__ = {
  ...
  methods: {
    ...
    getType(id) {
      ...
      watch(role, () => {
        console.log("Type = ", JSON.stringify(role));
    ...
    async upsertType(args) {
      try {
          ...
          const { data: res } = await useFetch(api + "create", {
            ...
            onResponse({ request, response, options }) {
              if (response._data.insertedId) {
                useNuxtApp().$toast.show({ type: "success", message: `Type [${response._data.insertedId}] added successfully`, timeout: 6 });
              }
            }
          }, "$QaBrtaDSzj");
        } else {
        ...
        this.progress = false;
        useNuxtApp().$bus.$emit("evtSearchType", { filter: {}, limit: 10 });
        this.data = {};
        this.isUpsertTypeVisible = !this.isUpsertTypeVisible;
      } catch (error) {
        console.log(error);
        this.progress = false;
      } finally {
    ...
    async deleteType() {
      try {
        await useFetch("/api/types/" + this.data._id, {
          method: "delete",
          body: {},
          initialCache: false,
          onResponse({ request, response, options }) {
            if (response._data.deletedCount == 1) {
              ...
              this.progress = false;
              this.isUpsertTypeVisible = false;
              useNuxtApp().$bus.$emit("evtSearchType", { filter: {}, limit: 10 });
              this.data = {};
            } else {
              this.progress = false;
    ...
    async closeTypePanel() {
      this.data = {};
      this.isUpsertTypeVisible = !this.isUpsertTypeVisible;
    },
    handleSelectedInType_record_status(e) {
      this.data.record_status = e.target.value;
    },
    handleSelectedInType_mark_as_delete(e) {
      this.data.mark_as_delete = e.target.value;
    }
  },
  created() {
    useNuxtApp().$bus.$on("evtUpsertType", (data) => {
      if (data !== void 0) {
        this.getType(data._cells[0].data);
      }
      this.isUpsertTypeVisible = !this.isUpsertTypeVisible;
    });
    useNuxtApp().$bus.$on("evtDeleteType", (data) => {
      if (data !== void 0) {
        this.data = data;
        this.deleteType();
      }
  ...
  beforeDestroy() {
    useNuxtApp().$bus.$off("evtUpsertType");
    useNuxtApp().$bus.$off("evtDeleteType");
  }
};
const _sfc_main = /* @__PURE__ */ Object.assign(__default__, {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (_ctx.isUpsertTypeVisible) {
        _push(`<section${ssrRenderAttrs(mergeProps({
          class: "fixed inset-0 overflow-hidden",
          "aria-labelledby": "slide-over-title",
          role: "dialog",
          "aria-modal": "true"
        }, _attrs))}><div class="absolute inset-0 overflow-hidden"><div class="absolute inset-0" aria-hidden="true"></div><div class="absolute inset-y-0 right-0 pl-10 max-w-full flex"><div class="w-screen max-w-md"><div class="h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl"><div class="flex-1 h-0 overflow-y-auto"><header class="space-y-1 py-6 px-4 bg-primary-200 bg-opacity-50 sm:px-6"><div class="flex items-center justify-between space-x-3"><h2 class="text-lg leading-7 font-medium text-gray-700">Type</h2><div class="h-7 flex items-center"><button aria-label="Close panel" class="rounded-md bg-primary-700 text-primary-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div></div><div><p class="text-sm leading-5 text-gray-500">Create or update Type</p></div></header>`);
        if (_ctx.progress) {
        ...
        if (!_ctx.progress) {
          _push(`<div class="flex-1 flex flex-col justify-between"><div class="px-4 divide-y divide-gray-200 sm:px-6"><div class="space-y-6 pt-6 pb-5"><form name="frmType" id="frmType" class="space-y-8 divide-y divide-gray-200"><div class="sm:overflow-hidden"><div class="bg-white space-y-6"><div class="grid grid-cols-1 gap-y-3 gap-x-4 sm:grid-cols-6"><div class="sm:col-span-6"><label for="first_name" class="block text-sm font-medium text-gray-700">Code</label><div class="mt-1"><input type="text" id="code" name="code"${ssrRenderAttr("value", _ctx.data.code)} class="py-3 px-4 block w-full pl-5 focus:ring-lime-600 focus:border-lime-600 border-gray-300"></div><p class="mt-2 text-sm text-gray-500">Custom code used as unique constraint. If the same string exists will be thrown duplicate record error.</p></div><div class="sm:col-span-6"><label for="first_name" class="block text-sm font-medium text-gray-700">Name or Value</label><div class="mt-1"><input type="text" id="name" name="name"${ssrRenderAttr("value", _ctx.data.name)} class="py-3 px-4 block w-full pl-5 focus:ring-lime-600 focus:border-lime-600 border-gray-300"></div></div><div class="sm:col-span-6">`);
          _push(ssrRenderComponent(_component_Dropdownlist, {
            data: { data: unref(record_status).documents },
            onChange: _ctx.handleSelectedInType_record_status,
            show_label: "true",
          ...
          _push(ssrRenderComponent(_component_Dropdownlist, {
            ...
            onChange: _ctx.handleSelectedInType_mark_as_delete,
            show_label: "true",
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/configurations/types/TypeUpsert.vue");
