...
let api = "/api/configurations/";
const __default__ = {
  components: {},
  data() {
    return {
      isUpsertConfigurationVisible: false,
  ...
  methods: {
    ...
    getConfiguration(id) {
      ...
      watch(role, () => {
        console.log("Configuration = ", JSON.stringify(role));
    ...
    async upsertConfiguration(args) {
      try {
        this.progress = true;
          ...
          const { data: res } = await useFetch(api + "create", {
            ...
            onResponse({ request, response, options }) {
              if (response._data.insertedId) {
                useNuxtApp().$toast.show({ type: "success", message: `Configuration [${response._data.insertedId}] added successfully`, timeout: 6 });
              }
            }
          }, "$EHvcY4LvpE");
        } else {
        ...
        this.progress = false;
        useNuxtApp().$bus.$emit("evtSearchConfiguration", { filter: {}, limit: 10 });
        this.data = {};
      } catch (error) {
        console.log(error);
        this.progress = false;
    ...
    async deleteConfiguration() {
      try {
        await useFetch("/api/configurations/" + this.data._id, {
          method: "delete",
          body: {},
          initialCache: false,
          onResponse({ request, response, options }) {
            if (response._data.deletedCount == 1) {
              ...
              this.progress = false;
              this.isUpsertConfigurationVisible = false;
              useNuxtApp().$bus.$emit("evtSearchConfiguration", { filter: {}, limit: 10 });
              this.data = {};
            } else {
              this.progress = false;
    ...
    async closeConfigurationPanel() {
      this.data = {};
      this.isUpsertConfigurationVisible = !this.isUpsertConfigurationVisible;
    },
  ...
  created() {
    useNuxtApp().$bus.$on("evtUpsertConfiguration", (data) => {
      if (data !== void 0) {
        this.getConfiguration(data._cells[0].data);
      }
      this.isUpsertConfigurationVisible = !this.isUpsertConfigurationVisible;
    });
    useNuxtApp().$bus.$on("evtDeleteConfiguration", (data) => {
      if (data !== void 0) {
        this.data = data;
  ...
  beforeDestroy() {
    useNuxtApp().$bus.$off("evtUpsertConfiguration");
    useNuxtApp().$bus.$off("evtDeleteConfiguration");
  }
};
const _sfc_main = /* @__PURE__ */ Object.assign(__default__, {
  __name: "ConfigurationUpsert",
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (_ctx.isUpsertConfigurationVisible) {
        _push(`<section${ssrRenderAttrs(mergeProps({
          class: "fixed inset-0 overflow-hidden",
          "aria-labelledby": "slide-over-title",
          role: "dialog",
          "aria-modal": "true"
        }, _attrs))}><div class="absolute inset-0 overflow-hidden"><div class="absolute inset-0" aria-hidden="true"></div><div class="absolute inset-y-0 right-0 pl-10 max-w-full flex"><div class="w-screen max-w-md"><div class="h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl"><div class="flex-1 h-0 overflow-y-auto"><header class="space-y-1 py-6 px-4 bg-primary-200 bg-opacity-50 sm:px-6"><div class="flex items-center justify-between space-x-3"><h2 class="text-lg leading-7 font-medium text-gray-700">Configuration</h2><div class="h-7 flex items-center"><button aria-label="Close panel" class="rounded-md bg-primary-700 text-primary-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div></div><div><p class="text-sm leading-5 text-gray-500">Create or update Configuration</p></div></header>`);
        if (_ctx.progress) {
        ...
        if (!_ctx.progress) {
          _push(`<div class="flex-1 flex flex-col justify-between"><div class="px-4 divide-y divide-gray-200 sm:px-6"><div class="space-y-6 pt-6 pb-5"><form name="frmConfiguration" id="frmConfiguration" class="space-y-8 divide-y divide-gray-200"><div class="sm:overflow-hidden"><div class="bg-white space-y-6"><div class="grid grid-cols-1 gap-y-3 gap-x-4 sm:grid-cols-6"><div class="sm:col-span-6"><h2 class="font-medium text-primary-700">Config Information </h2></div><div class="sm:col-span-6">`);
          ...
          _push(`</div><div class="sm:col-span-6"><label for="code" class="block text-sm font-medium text-gray-700">Code</label><div class="mt-1"><input type="text" id="code" name="code"${ssrRenderAttr("value", _ctx.data.code)} class="py-3 px-4 block w-full pl-5 focus:ring-lime-600 focus:border-lime-600 border-gray-300"><p class="mt-2 text-sm text-gray-500">An unique string within each property type. If the same string exists within the selected property type, error will be thrown!</p></div></div><div class="sm:col-span-6"><label for="name" class="block text-sm font-medium text-gray-700">Name or Value</label><div class="mt-1"><input type="text" id="name" name="name"${ssrRenderAttr("value", _ctx.data.name)} class="py-3 px-4 block w-full pl-5 focus:ring-lime-600 focus:border-lime-600 border-gray-300"><p class="mt-2 text-sm text-gray-500">This can be any valid string which includes speical characters, numeric values.</p></div></div><div class="sm:col-span-6"><h2 class="font-medium text-primary-700">Record Information </h2></div><div class="sm:col-span-6">`);
          _push(ssrRenderComponent(_component_Dropdownlist, {
            data: { data: unref(record_status).documents },
            onChange: _ctx.handleSelectedInConfiguration_record_status,
            show_label: "true",
          ...
          _push(ssrRenderComponent(_component_Dropdownlist, {
            ...
            onChange: _ctx.handleSelectedInConfiguration_mark_as_delete,
            show_label: "true",
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/configurations/ConfigurationUpsert.vue");
