...
import { E as usePropertiesStore, d as useNuxtApp, F as useFetch } from '../server.mjs';
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    useNuxtApp().$bus.$on("evtDeleteProperties", (data2) => {
      deleteProperties(data2._cells[0].data);
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(isUpsertPropertiesVisible)) {
        _push(`<section${ssrRenderAttrs(mergeProps({
          class: "fixed inset-0 overflow-hidden",
          "aria-labelledby": "slide-over-title",
          role: "dialog",
          "aria-modal": "true"
        }, _attrs))}><div class="absolute inset-0 overflow-hidden"><div class="absolute inset-0" aria-hidden="true"></div><div class="absolute inset-y-0 right-0 pl-10 max-w-full flex"><div class="w-screen max-w-md"><div class="h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl"><div class="flex-1 h-0 overflow-y-auto"><header class="space-y-1 py-6 px-4 bg-primary-700 sm:px-6"><div class="flex items-center justify-between space-x-3"><h2 class="text-lg leading-7 font-medium text-white">Properties</h2><div class="h-7 flex items-center"><button aria-label="Close panel" class="rounded-md bg-primary-700 text-primary-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white"><span class="sr-only">Close panel</span>`);
        _push(ssrRenderComponent(_component_Icon, {
          name: "material-symbols:close",
          class: "h-6 w-6",
          "aria-hidden": "true"
        }, null, _parent));
        _push(`</button></div></div><div><p class="text-sm leading-5 text-primary-300">Manage custom properties used as configuration varables</p></div></header>`);
        if (unref(progress)) {
        ...
        if (!unref(progress)) {
          _push(`<div class="flex-1 flex flex-col justify-between"><div class="px-4 divide-y divide-gray-200 sm:px-6"><div class="space-y-6 pt-6 pb-5"><form name="frmProperties" id="frmProperties" class="space-y-8 divide-y divide-gray-200"><div class="sm:overflow-hidden"><div class="bg-white space-y-6"><div class="grid grid-cols-1 gap-y-3 gap-x-4 sm:grid-cols-6"><div class="sm:col-span-6">`);
          _push(ssrRenderComponent(_component_Dropdownlist, {
            data: { data: unref(propertiesStore).parent_properties },
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/properties/PropertiesUpsert.vue");
