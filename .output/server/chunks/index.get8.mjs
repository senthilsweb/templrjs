...
const index_get = eventHandler(async (event) => {
  ...
  console.log("Query=", `${process.env.SUPABASE_URL}/rest/v1/instagram_posts?${lodash.join(query_string, "&")}`);
  const data = await $fetch(`${process.env.SUPABASE_URL}/rest/v1/instagram_posts?${lodash.join(query_string, "&")}`, {
    method: "get",
    headers: {
