      ...
      <div class="sm:text-center md:mx-auto md:max-w-2xl lg:col-span-6 lg:text-left">
          ...
          <span class="mt-1 p-1 block text-4xl space-y-6 font-bold tracking-tight sm:text-2xl xl:text-5xl">
            ...
            <span
              class="block text-transparent bg-clip-text bg-gradient-to-r from-green-400 via-blue-500 to-purple-500">{{
                hero.value.title.secondary }}</span>
          </span>
        </h1>
        ...
        <div class="mt-10 sm:flex sm:justify-center lg:justify-start">
          <div class="hidden md:flex md:items-center md:space-x-6">
            <NuxtLink to="#" @click="useNuxtApp().$bus.$emit(`${hero.value.button.primary.click_event}`)"
              ...
              {{ hero.value.button.primary.text }}</NuxtLink>
            <NuxtLink to="#" @click="useNuxtApp().$bus.$emit(`${hero.value.button.secondary.click_event}`)"
              ...
              {{ hero.value.button.secondary.text }}</NuxtLink>
          </div>
        </div>
      </div>
      <div
