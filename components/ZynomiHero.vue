      ...
      <div class="sm:text-center md:mx-auto md:max-w-2xl lg:col-span-6 lg:text-left">
        <h1>
          <span class="mt-1 p-1 block text-4xl space-y-6 font-bold tracking-tight sm:text-2xl xl:text-5xl">
            <span class="block text-gray-900">Rollout your internal tool</span>
            <span class="block text-transparent bg-clip-text bg-gradient-to-r from-green-400 via-blue-500 to-purple-500">with extraordinary velocity</span>
          </span>
        </h1>
        <p class="mt-3 text-base text-gray-700 sm:mt-5 sm:text-sm lg:text-lg xl:text-lg">Build high-performant web and mobile optimized sites and apps, data-driven dashboards, and modern front end for legacy systems on-prem and cloud with zero to low development and operating cost!!!</p>

        <div class="mt-10 sm:flex sm:justify-center lg:justify-start">
          <div class="hidden md:flex md:items-center md:space-x-6">
            <NuxtLink to="#" @click="useNuxtApp().$bus.$emit(`${primary_button.click_event}`)" class="px-5 py-2 text-md ring-2 hover:ring-2 text-white rounded-full transition-all duration-300" :class="[`bg-[${useThemeStore().palette('500')}]`, `hover:bg-[${useThemeStore().palette('700')}]`, `ring-[${useThemeStore().palette('500')}]`]"> Get Started</NuxtLink>
            <NuxtLink to="#" @click="useNuxtApp().$bus.$emit(`${secondary_button.click_event}`)" class="px-5 py-2 text-md border border-transparent bg-white hover:text-white ring-2 rounded-full transition-all duration-300" :class="[`text-[${useThemeStore().palette('500')}]`, `hover:bg-[${useThemeStore().palette('700')}]`, `ring-[${useThemeStore().palette('500')}]`]"> Contact Us</NuxtLink>
          </div>
        </div>
      </div>
...
console.log("primary_button=",primary_button);
const secondary_button = useFilter(navs, { code: 'secondary-button' })[0];
console.log("secondary_button=",secondary_button);
