<template>
  ...
  >
    ...
    <div class="relative w-full overflow-hidden after:clear-both after:block after:content-[''] px-4 sm:px-0">
      <div
        v-for="(item, idx) in navigation.data"
        :key="idx"
...
const { data: navigation } = await useAsyncData('carousel-list', () => {
  return queryContent().where({ _path: '/_navigation/carousel' }).findOne();
});
c_total.value = navigation.value.data.length;
const carousel = useState('carousel', () => ref({}));
navigation.value.data.forEach((item, idx) => {
